...
{$IFDEF FPC} {$DEFINE TABLECRC} {$ENDIF}
{$IFDEF VIRTUALPASCAL} {$DEFINE TABLECRC} {$ENDIF}
{$IFDEF TABLECRC}
...
Procedure CalcCRC (var S: String);
{ Same CRC-16/CCITT over the line and its CR/LF as the assembler below, off
  a table written by Alexey Korotkov (2:455/19).  The assembler cannot be
  built for 64-bit targets, and this is faster, but its 512-byte table does
  not fit in the 64K data segment shared by the whole DOS16 program -- hence
  the two.  One thing not carried over: the assembler writes that CR/LF into
  the string buffer past Length(S), which nothing reads back and which
  overruns a full-length string. }
Var
  i: Integer;
Begin
  For i := 1 To Length (S) Do CRC := CRC16DoByte (Ord (S [i]), CRC);
  CRC := CRC16DoByte (13, CRC);
  CRC := CRC16DoByte (10, CRC);
End;
{$ELSE}
{ 1.00's assembler, kept for Borland Pascal: see above for why.  The
  VIRTUALPASCAL half is 1.00's too and is now unreachable -- Virtual Pascal
  takes the table -- but it is left where it was rather than thrown away. }
Procedure CalcCRC (var S: String);
Begin
  Asm
