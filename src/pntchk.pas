...
{$IFDEF FPC}{$MODESWITCH EXCEPTIONS}{$ENDIF}
{$IFDEF FPC}{$CODEPAGE CP866}{$ENDIF}
...
(* {$S+} *)
Program PNTCHK;
Uses {$IFDEF FPC} sysutils,
     {$ELSE} {$IFDEF LINUX} {$IFDEF BSD} linux, {$ELSE} unix, {$ENDIF} {$ENDIF} dos,
     {$ENDIF}
     {$IFDEF BW} CrtFake, {$ELSE} Crt, {$ENDIF}
...
Const
  NameProg = 'PNTCHK'
      {$IFDEF WIN32} +'/W32' {$ENDIF} {$IFDEF WIN64} +'/W64' {$ENDIF}
      {$IFDEF DPMI} +'/32' {$ENDIF} {$IFDEF DOS32} +'/32' {$ENDIF}
      {$IFDEF LINUX} +'/LNX' {$ENDIF}
      {$IFDEF DARWIN} +'/MAC' {$ELSE}
      {$IFDEF NETBSD} +'/NBSD' {$ELSE}
      {$IFDEF OPENBSD} +'/OBSD' {$ELSE}
      {$IFDEF FREEBSD} +'/FBSD' {$ELSE}
      {$IFDEF BSD} +'/BSD' {$ENDIF} {$ENDIF} {$ENDIF} {$ENDIF} {$ENDIF}
      {$IFDEF OS2}{$IFDEF EMX} +'/EMX' {$ELSE} +'/2' {$ENDIF}{$ENDIF} ;
{$IFDEF PLATWIN} namefprog = 'PNTCHKW.EXE'; {$DEFINE GOTNAME} {$ENDIF}
{$IFDEF PLATEMX} namefprog = 'PNTCHKE.EXE'; {$DEFINE GOTNAME} {$ENDIF}
{$IFDEF PLATD32} namefprog = 'PNTCHKP.EXE'; {$DEFINE GOTNAME} {$ENDIF}
{$IFDEF PLATLNX} {$IFDEF BW} namefprog = 'PNTCHK_B.LNX';
                 {$ELSE} namefprog = 'PNTCHK_C.LNX'; {$ENDIF}
                 {$DEFINE GOTNAME} {$ENDIF}
{$IFDEF PLATBSD} {$IFDEF BW} namefprog = 'PNTCHK_B.BSD';
                 {$ELSE} namefprog = 'PNTCHK_C.BSD'; {$ENDIF}
                 {$DEFINE GOTNAME} {$ENDIF}
{$IFNDEF GOTNAME} namefprog = 'PNTCHK.EXE'; {$ENDIF}
{  Version = '1.00+ (final)';}
  Version = '1.01';
{$IFDEF FPC}
...
{$ENDIF}
   ...
   unixlines,wasattr,endflag,eoff,registered: boolean;
   pidserial: string[16];
{   namefprog: string;}
...
begin
  ...
  if (a1<>0)
   ...
   begin
      ...
      if (st1<>'') {$IFDEF FPC}and (st1[length(st1)]<>'/') {$ENDIF} and (st1[length(st1)]<>'\')
        then st1:=st1+ {$IFDEF LINUX} '/' {$ELSE} '\' {$ENDIF} ;
...
{   textcolor(7); write(' ');}
  ...
  if str1<>'' then
   if (str1<>'') {$IFDEF FPC}and (str1[length(str1)]<>'/') {$ENDIF} and (str1[length(str1)]<>'\')
      then str1:=str1+ {$IFDEF LINUX} '/'
                                        {$ELSE} '\' {$ENDIF} ;
  str2:='MSG_TMP.FFF';
...
begin
 ...
 if ioresult<>0 then
   begin
     str8:=getctlvaluelite('TEMPLATEPATH');
     if (str8<>'') {$IFDEF FPC}and (str8[length(str8)]<>'/') {$ENDIF} and (str8[length(str8)]<>'\')
      then str8:=str8+ {$IFDEF LINUX}
'/' {$ELSE}
'\'{$ENDIF} ;
     str8:=str8+tplnm;
     assign(tpfl^ [1],str8);
  ...
  begin
   ...
   if b1<>0 then
    begin
     ...
     if ioresult<>0 then begin
      str7:=getctlvaluelite('TEMPLATEPATH');
      if (str7<>'') {$IFDEF FPC}and (str7[length(str7)]<>'/') {$ENDIF} and (str7[length(str7)]<>'\')
      then str7:=str7+ {$IFDEF LINUX}
'/' {$ELSE} '\' {$ENDIF};
...
{   Net.SetPvt(True);
   Net.SetLoc(True);}
   setmsgflags;
   Net.MsgText.Putstring('PID: '+nameprog+' v'+version+pidserial);
...
if not closev then begin
 str1:=getctlvaluelite('TEMPLATEPATH');
 if (str1<>'') {$IFDEF FPC}and (copy(str1,length(str1),1)<>'/') {$ENDIF} and (copy(str1,length(str1),1)<>'\')
...
{ if strupcase(tplname)<>'PNTCHK.EXE' then}
 ...
 runerror('error',2);
  ...
  st1:= getctlvalue('TEMPDIR');
  if st1<>''
      then if (st1<>'') {$IFDEF FPC}and (st1[length(st1)]<>'/') {$ENDIF}
            and (st1[length(st1)]<>'\') then st1:=st1+ {$IFDEF LINUX} '/'
                                        {$ELSE} '\' {$ENDIF} ;
{  st2:= getctlvalue('TEMPFILE');
  if st2='' then} st2:='MSG_TMP.FFF';
  assign(tempfl2 , st1+st2);
...
{$IFNDEF WIN32} if strupcase(copy(param1,1,2))<>strupcase(copy(param2,1,2)) then
 {$ENDIF}
     begin
  Reset(FromF, 1);  { Record size = 1 }
...
begin { genuine readsegment }
 ...
 if pth1<>'' then begin
  ...
  chdir(ExtractFileDir(IncludeTrailingPathDelimiter(pth1)));
  if IOResult <> 0 then haltonerror(
...
{     tplar[15]:=postr;}
 ...
 st2:= getctlvalue('TEMPDIR');
 if st2<>''
      then if (st2<>'') {$IFDEF FPC}and (st2[length(st2)]<>'/') {$ENDIF}
            and (st2[length(st2)]<>'\') then st2:=st2+{$IFDEF LINUX}
'/' {$ELSE}
'\' {$ENDIF} ;
 st3:='PNT_TMP.FFF';
...
{  if copy(param2,2,1)=':' then begin}
{   getdir(0,str1);}
(*   if (str1<>'') {$IFDEF FPC}and (str1[length(str1)]<>'/') {$ENDIF} and (str1[length(str1)]<>'\') then str1:=str1+
     {$IFDEF LINUX} '/' {$ELSE} '\' {$ENDIF} ; *)
...
{$IFNDEF WIN32}  if strupcase(copy(tplar[33],1,2))<>strupcase(copy(param2,1,2)) then
 {$ENDIF}
     begin
...
'\' {$ENDIF} ) and (copy(pth1,2,length(pth1)-1)<>':\') then delete(pth1,length(pth1),1);
 ...
 if tplar[11] {@result} = 'ok' then
   begin
     ...
     if pth1<>'' then begin { pth1:=pth2;}
     if (pth1<>'') {$IFDEF FPC}and (copy(pth1,length(pth1),1)<>'/') {$ENDIF} and (copy(pth1,length(pth1),1)<>'\')
      then pth1:=pth1+{$IFDEF LINUX}
'/' {$ELSE}
'\' {$ENDIF} ;
   ...
   if (strupcase(copy(getctlvaluelite('KILLBAD'),1,1))='I') or
      (strupcase(copy(getctlvaluelite('KILLBAD'),1,1))='A') or
      (strupcase(copy(getctlvaluelite('KILLBAD'),1,1))='Y') then
     begin
      pth1:= GetCTLValuelite ('BADFILES');
      if pth1='' then pth1:=pth2;
      if (pth1<>'') {$IFDEF FPC}and (pth1[length(pth1)]<>'/') {$ENDIF} and (pth1[length(pth1)]<>'\')
      then pth1:=pth1+{$IFDEF LINUX}
'/' {$ELSE}
'\' {$ENDIF}
;
...
{ no key, no complaint: registration is optional }
 ...
 for i:=1 to paramcount do if (copy(paramstr(i),1,1)='-')
  ...
  if (paramcount=0) or (str4='') then
    begin
      textcolor(15);
      write('Usage: '+namefprog+' [-c<config>] <option> [<segment_file>]');
      writeln;
      writeln;
      writeln('<options>');
...
{ ---------- checkdirs ----------------- }
  ...
  if str3<>'' then begin
   chdir(ExtractFileDir(IncludeTrailingPathDelimiter(str3)));
   if IOResult <> 0 then begin
     chdir(str2);
     haltonerror(
...
logmsg('@','Scanning MASTER directory : '+nametest);
	...
	(getctlvalue('CREATIONCHECK')='') then str3:='MEDIUM' else
      ...
      if (nametest<>'') {$IFDEF FPC}and (nametest[length(nametest)]<>'/') {$ENDIF} and (nametest[length(nametest)]<>'\')
        then nametest:=nametest+{$IFDEF LINUX}
'/' {$ELSE}
'\' {$ENDIF}
;
